...
IV. Desired functionality
-------------------------

Top priority:

1. Ability to download/upload “data files” so that data can be collected from computers that don’t have an internet connection.
2. Make AppCache “application update” more user-friendly and secure. Need to "freeze" the user as soon as a system update (e.g. form change) is ready so that he/she is forced to download the update before proceeding. Also, the AppCache update code should show download progress.
3. Ability to download/upload "system files" so that application updates can be installed on computers that don’t have an internet connection (stopgap solution will be to swap laptops between data entry clerks).


Mid priority:

1. Make “Send Records” interface more user-friendly (e.g. give progress messages and more helpful error codes).
2. Expand user account types and build out privilege sets.
3. Create a form-builder tool to allow non-technical users to create and modify data-collection forms.
4. Create admin interface (e.g. add/modify/delete user accounts).
5. Create data manager interface (e.g. enter FHW retention data).
6. Build or integrate a data reporting/visualization interface.


Low priority:

1. Improve system responsiveness for small laptops and Android phones.
2. Modify App so that it works on browsers other than Google Chrome.
3. Expand form validation to include additional validation options.
