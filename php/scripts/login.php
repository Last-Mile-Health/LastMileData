...
$query = "SELECT * FROM lastmile_dataportal.tbl_utility_users WHERE username = '$input_username' AND archived<>1";
...
if( $input_username <> " AND $input_password <> " AND sha1($input_password) == $row['password'] )
{
    ...
    // Redirect
    if ( $redirect == "" ) {
        Header("Location:/LastMileData/build/pages/page_home.php");
    } else {
